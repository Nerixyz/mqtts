...
describe('PacketWriter', function() {
    it('should write an empty packet', function() {
        assertBuffer(new PacketWriter({logPacketWrite: () => undefined }, {
            [PacketType.Connect]: () => {
                return {}
            }
        }).write(PacketType.Connect), '1000');
    });
    it('should write flags', function() {
        ...
        }).write(PacketType.Connect), '1200');
    });
    it('should throw on invalid flags', function() {
        assert.throws(() => new PacketWriter({logPacketWrite: () => undefined }, {
            [PacketType.Connect]: () => {
                return {flags: 0xf0}
            }
    ...
    it('should write a proper remaining length', function() {
        ...
        }).write(PacketType.Connect), '10080123456789abcdef');
    });
